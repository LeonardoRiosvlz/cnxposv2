...
type SubgroupFormFiles = {
    //resolutionFile?: CloudFile,
    //programFile?: CloudFile
}

type Props = {
    submitAction: (data: SubgroupFormField) => unknown
    formId: string
    initialData?: SubgroupFormField
    initialFiles?: SubgroupFormFiles,
    upLoading?: () => void,
}
const SubgroupForm: React.FC<Props> = ({submitAction, initialFiles, upLoading, formId, initialData}: Props) => {
    ...
    const defaultValues: SubgroupFormField | {} = initialData ? {
        ...initialData
    } : {isActive: true};

    const {handleSubmit, control, formState: {errors, dirtyFields, isValid}} = useForm<SubgroupFormField>({
        mode: 'onChange',
        // @ts-ignore
    ...
    //const [files, setFiles] = React.useState<{ [K in keyof SubgroupFormFiles]?: File }>({});
    ...
    const onSubmit = (data: SubgroupFormField) => {
