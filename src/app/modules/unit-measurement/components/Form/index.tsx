...
export type UnitMeasurementFormField = {
    name: string;
    description?: string;
...
type UnitMeasurementFormFiles = {
    //resolutionFile?: CloudFile,
    //programFile?: CloudFile
}

type Props = {
    submitAction: (data: UnitMeasurementFormField) => unknown
    formId: string
    initialData?: UnitMeasurementFormField
    initialFiles?: UnitMeasurementFormFiles,
    upLoading?: () => void,
}
const UnitMeasurementForm: React.FC<Props> = ({submitAction, initialFiles, upLoading, formId, initialData}: Props) => {
    ...
    const defaultValues: UnitMeasurementFormField | {} = initialData ? {
        ...initialData
    } : {};

    const {handleSubmit, control, formState: {errors, dirtyFields, isValid}} = useForm<UnitMeasurementFormField>({
        mode: 'onChange',
        // @ts-ignore
    ...
    //const [files, setFiles] = React.useState<{ [K in keyof UnitMeasurementFormFiles]?: File }>({});
    ...
    const onSubmit = (data: UnitMeasurementFormField) => {
