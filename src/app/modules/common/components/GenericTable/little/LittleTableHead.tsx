...
import { TableRecord } from 'app/modules/common/components/GenericTable/store/generic-table.slice';
import { useAppSelector } from 'app/modules/common/hooks/use-app-redux';
import { GenericTableLittleState } from '../store/generic-table-little.slice';
...
type LittleTableHeadProps<T> = {
	rows: Array<RowsDef<T>>,
	onRequestSort?: (property: string) => void;
	onSelectAllClick?: (event: React.ChangeEvent<HTMLInputElement>) => void;
	onCheckSelectMenuClick?: (event?: React.ChangeEvent<HTMLInputElement>, row?: T) => void;
	actionColumn?: boolean
}

const LittleTableHead = <T,>({
	rows,
	onRequestSort,
	onSelectAllClick,
	onCheckSelectMenuClick,
	actionColumn
}: LittleTableHeadProps<T>) => {
	const classes = useStyles();
	const { t } = useTranslation('common');
	const { paginator, items, selected } = useAppSelector(state => state.table_little) as GenericTableLittleState;
				...
				<TableCell padding="none" className="w-40 md:w-64 text-center z-99">
					...
					{numSelected > 0 && onCheckSelectMenuClick && (
						...
						>
							...
							>
								<MenuList>
									<MenuItem onClick={() => onCheckSelectMenuClick()}>
...
export default LittleTableHead;
