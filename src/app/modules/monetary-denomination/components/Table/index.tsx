...
import MonetaryDenominationForm, {MonetaryDenominationFormField} from '../Form';
import MonetaryDenominationDetails from '../Details';
...
import {MonetaryDenominationRepository} from "api/repositories/monetary-denomination/monetary-denomination.repository";
...
type Props = {}
const MonetaryDenominationTable: React.FC<Props> = (props: Props) => {
    ...
    const monetaryDenominationRepo = useRepository(MonetaryDenominationRepository)
    ...
    const selectedItem = _.first(selected) as MonetaryDenomination;
    ...
    React.useEffect(() => {
        // @ts-ignore
        dispatch(setContext(App_Modules.MonetaryDenomination));
    }, []);
    ...
    const fetch = () => {
        ...
        monetaryDenominationRepo.findPaginated(
            {
                input: {
            ...
            }).then((res: IPaginatedEntity<MonetaryDenomination>) => {
            // @ts-ignore
    ...
    const addAction = (formData: MonetaryDenominationFormField) => {
        setCrudLoading({...crudLoading, add: true});
        monetaryDenominationRepo.create({input: {...formData}}).then(() => {
    ...
    const editAction = (formData: MonetaryDenominationFormField) => {
        setCrudLoading({...crudLoading, edit: true});

        const toUpdateData = FormUtils.toUpdateValue<MonetaryDenominationFormField, MonetaryDenomination>(formData, selectedItem);

        monetaryDenominationRepo.update({
            input: {
    ...
    const removeAction = () => {
        ...
        monetaryDenominationRepo.delete({input: {entityId: selectedItem?.id}}).then(() => {
    ...
    const removeMultipleAction = () => {
        ...
        monetaryDenominationRepo.deleteMany({input: {where: {id: {in: ids}}}}).then(() => {
            setCrudLoading({...crudLoading, delete: false});
    ...
    const rows: Array<RowsDef<MonetaryDenomination>> = [
        {
            id: 'value',
            align: 'left',
            sort: true,
        ...
        {
            id: 'monetaryDenominationTypes',
            align: 'left',
            sort: true,
            label: t('MONETARY_DENOMINATION_TYPES'),
            render: (row) =><label>{t(row.monetaryDenominationTypes)}</label>
        },
        {
            id: 'photoFile',
            align: 'left',
    ...
    const formId = {
        edit: `edit-monetary-denomination`,
        add: `add-monetary-denomination`
    };
    ...
    return (<>
            <div className='w-full flex flex-col'>
                <GenericTable rows={rows}
                              ...
                              title={t('common:MONETARY_DENOMINATION')}
            ...
            <GenericModal onClose={() => handleCloseDialog('view')}
                ...
                <div className='w-full p-4'>
                    <MonetaryDenominationDetails row={selectedItem}/>
                </div>
            </GenericModal>
            ...
            <GenericModal maxWidth={'sm'} minHeight={200}
                          ...
                          title={`${t('common:EDIT')} : ${selectedItem?.value}`}
                ...
                <div className='w-full p-4 mt-16'>
                    <MonetaryDenominationForm
                        ...
                        initialData={{
                        ...FormUtils.setInitialData<MonetaryDenomination, MonetaryDenominationFormField>(selectedItem)
                    }}
...
export default MonetaryDenominationTable;
