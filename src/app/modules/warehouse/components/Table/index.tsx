...
import WarehouseForm, {WarehouseFormField} from '../Form';
...
type Props = {}
const WarehouseTable: React.FC<Props> = (props: Props) => {
    ...
    const selectedItem = _.first(selected) as Warehouse;
    ...
    React.useEffect(() => {
        // @ts-ignore
        dispatch(setContext(App_Modules.Warehouse));
    }, []);
    ...
    const fetch = () => {
        ...
        warehouseRepo.findPaginated(
            ...
            }).then((res: IPaginatedEntity<Warehouse>) => {
            // @ts-ignore
    ...
    const addAction = (formData: WarehouseFormField) => {
        setCrudLoading({...crudLoading, add: true});
        warehouseRepo.create({input: {...formData}}).then(() => {
    ...
    const editAction = (formData: WarehouseFormField) => {
        setCrudLoading({...crudLoading, edit: true});

        const toUpdateData = FormUtils.toUpdateValue<WarehouseFormField, Warehouse>(formData, selectedItem);
    ...
    const removeAction = () => {
        ...
        warehouseRepo.delete({input: {entityId: selectedItem?.id}}).then(() => {
    ...
    const removeMultipleAction = () => {
        ...
        warehouseRepo.deleteMany({input: {where: {id: {in: ids}}}}).then(() => {
            setCrudLoading({...crudLoading, delete: false});
    ...
    const rows: Array<RowsDef<Warehouse>> = [
        {
            id: 'name',
            align: 'left',
            sort: true,
        ...
        },
        {
            ...
            render: (record) => <label>{t(record.warehouseTypes)}</label> 
        },
        {
            id: 'description',
            align: 'left',
    ...
    return (<>
            ...
            <GenericModal onClose={() => handleCloseDialog('view')}
                ...
                <div className='w-full p-4'>
                    <WarehouseDetails row={selectedItem}/>
                </div>
            </GenericModal>
            ...
            <GenericModal maxWidth={'sm'} minHeight={200}
                ...
                <div className='w-full p-4 mt-16'>
                    <WarehouseForm
                        ...
                        initialData={{
                        ...FormUtils.setInitialData<Warehouse, WarehouseFormField>(selectedItem)
                    }}
