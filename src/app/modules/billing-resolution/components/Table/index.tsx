...
import BillingResolutionForm, {BillingResolutionFormField} from '../Form';
import BillingResolutionDetails from '../Details';
...
type Props = {}
const BillingResolutionTable: React.FC<Props> = (props: Props) => {
    ...
    const selectedItem = _.first(selected) as BillingResolution;
    ...
    React.useEffect(() => {
        // @ts-ignore
        dispatch(setContext(App_Modules.BillingResolution));
    }, []);
    ...
    const fetch = () => {
        ...
        billingResolutionRepo.findPaginated(
            {
                input: {
                    paginator: {limit: paginatorInput.rowsPerPage, page: paginatorInput.page},
                    where: query ? {or: [{resolutionNumber: {iLike: query}}]} : {}
                }
            }).then((res: IPaginatedEntity<BillingResolution>) => {
            // @ts-ignore
    ...
    const addAction = (formData: BillingResolutionFormField) => {
        setCrudLoading({...crudLoading, add: true});
        billingResolutionRepo.create({input: {...formData}}).then(() => {
    ...
    const editAction = (formData: BillingResolutionFormField) => {
        setCrudLoading({...crudLoading, edit: true});

        const toUpdateData = FormUtils.toUpdateValue<BillingResolutionFormField, BillingResolution>(formData, selectedItem);

        billingResolutionRepo.update({
            input: {
    ...
    const removeAction = () => {
        ...
        billingResolutionRepo.delete({input: {entityId: selectedItem?.id}}).then(() => {
    ...
    const removeMultipleAction = () => {
        ...
        billingResolutionRepo.deleteMany({input: {where: {id: {in: ids}}}}).then(() => {
            setCrudLoading({...crudLoading, delete: false});
    ...
    const rows: Array<RowsDef<BillingResolution>> = [
        {
            id: 'resolutionClass',
            align: 'left',
            sort: true,
    ...
    const formId = {
        edit: `edit-billing-resolution`,
        add: `add-billing-resolution`
    };
    ...
    return (<>
            <div className='w-full flex flex-col'>
                <GenericTable rows={rows}
                              ...
                              title={t('common:BILLING_RESOLUTION')}
            ...
            <GenericModal onClose={() => handleCloseDialog('view')}
                ...
                <div className='w-full p-4'>
                    <BillingResolutionDetails row={selectedItem}/>
                </div>
            </GenericModal>
            ...
            <GenericModal maxWidth={'sm'} minHeight={200}
                          ...
                          title={`${t('common:EDIT')} : ${selectedItem?.resolutionNumber}`}
                ...
                <div className='w-full p-4 mt-16'>
                    <BillingResolutionForm
                        ...
                        initialData={{
                        ...FormUtils.setInitialData<BillingResolution, BillingResolutionFormField>(selectedItem)
                    }}
...
export default BillingResolutionTable;
