...
import PreparationForm, {PreparationFormField} from '../Form';
import PreparationDetails from '../Details';
...
type Props = {}
const PreparationTable: React.FC<Props> = (props: Props) => {
    ...
    const selectedItem = _.first(selected) as Preparation;
    ...
    React.useEffect(() => {
        // @ts-ignore
        dispatch(setContext(App_Modules.Preparation));
    }, []);
    ...
    const fetch = () => {
        ...
        preparationRepo.findPaginated(
            ...
            }).then((res: IPaginatedEntity<Preparation>) => {
            // @ts-ignore
    ...
    const addAction = (formData: PreparationFormField) => {
        setCrudLoading({...crudLoading, add: true});
        preparationRepo.create({input: {...formData}}).then(() => {
    ...
    const editAction = (formData: PreparationFormField) => {
        setCrudLoading({...crudLoading, edit: true});

        const toUpdateData = FormUtils.toUpdateValue<PreparationFormField, Preparation>(formData, selectedItem);

        preparationRepo.update({
            input: {
    ...
    const removeAction = () => {
        ...
        preparationRepo.delete({input: {entityId: selectedItem?.id}}).then(() => {
    ...
    const removeMultipleAction = () => {
        ...
        preparationRepo.deleteMany({input: {where: {id: {in: ids}}}}).then(() => {
            setCrudLoading({...crudLoading, delete: false});
    ...
    const rows: Array<RowsDef<Preparation>> = [
        {
            id: 'name',
            align: 'left',
            sort: true,
    ...
    return (<>
            ...
            <GenericModal onClose={() => handleCloseDialog('view')}
                ...
                <div className='w-full p-4'>
                    <PreparationDetails row={selectedItem}/>
                </div>
            </GenericModal>
            ...
            <GenericModal maxWidth={'sm'} minHeight={200}
                ...
                <div className='w-full p-4 mt-16'>
                    <PreparationForm
                        ...
                        initialData={{
                        ...FormUtils.setInitialData<Preparation, PreparationFormField>(selectedItem)
                    }}
