...
import PreparationTypeForm, {PreparationTypeFormField} from '../Form';
import PreparationTypeDetails from '../Details';
import {PreparationType} from 'api/repositories/preparation-type/types/preparation-type.types';
...
import {PreparationTypeRepository} from "api/repositories/preparation-type/preparation-type.repository";
...
type Props = {}
const PreparationTypeTable: React.FC<Props> = (props: Props) => {
    ...
    const preparationTypeRepo = useRepository(PreparationTypeRepository)
    ...
    const selectedItem = _.first(selected) as PreparationType;
    ...
    React.useEffect(() => {
        // @ts-ignore
        dispatch(setContext(App_Modules.PreparationType));
    }, []);
    ...
    const fetch = () => {
        ...
        preparationTypeRepo.findPaginated(
            {
                input: {
            ...
            }).then((res: IPaginatedEntity<PreparationType>) => {
            // @ts-ignore
    ...
    const addAction = (formData: PreparationTypeFormField) => {
        setCrudLoading({...crudLoading, add: true});
        preparationTypeRepo.create({input: {...formData}}).then(() => {
    ...
    const editAction = (formData: PreparationTypeFormField) => {
        setCrudLoading({...crudLoading, edit: true});

        const toUpdateData = FormUtils.toUpdateValue<PreparationTypeFormField, PreparationType>(formData, selectedItem);

        preparationTypeRepo.update({
            input: {
    ...
    const removeAction = () => {
        ...
        preparationTypeRepo.delete({input: {entityId: selectedItem?.id}}).then(() => {
    ...
    const removeMultipleAction = () => {
        ...
        preparationTypeRepo.deleteMany({input: {where: {id: {in: ids}}}}).then(() => {
            setCrudLoading({...crudLoading, delete: false});
    ...
    const rows: Array<RowsDef<PreparationType>> = [
        {
            id: 'name',
            align: 'left',
            sort: true,
    ...
    const formId = {
        edit: `edit-preparation-type`,
        add: `add-preparation-type`
    };
    ...
    return (<>
            <div className='w-full flex flex-col'>
                <GenericTable rows={rows}
                              ...
                              title={t('common:PREPARATION_TYPE')}
            ...
            <GenericModal onClose={() => handleCloseDialog('view')}
                ...
                <div className='w-full p-4'>
                    <PreparationTypeDetails row={selectedItem}/>
                </div>
            </GenericModal>
            ...
            <GenericModal maxWidth={'sm'} minHeight={200}
                ...
                <div className='w-full p-4 mt-16'>
                    <PreparationTypeForm
                        ...
                        initialData={{
                        ...FormUtils.setInitialData<PreparationType, PreparationTypeFormField>(selectedItem, ['preparation']),
                        preparation: Array.from(selectedItem?.preparation ?? []).map(x => x.id)
                    }}
