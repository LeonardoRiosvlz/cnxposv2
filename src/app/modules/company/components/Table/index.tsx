...
import CompanyForm, {CompanyFormField} from '../Form';
...
import {CompanyRepository} from "api/repositories/company/company.repository";
...
const CompanyTable: React.FC<Props> = (props: Props) => {
    ...
    const companyRepo = useRepository(CompanyRepository)
    ...
    const selectedItem = _.first(selected) as Company;
    ...
    React.useEffect(() => {
        // @ts-ignore
        dispatch(setContext(App_Modules.Company));
    }, []);
    ...
    const fetch = () => {
        ...
        companyRepo.findPaginated(
            ...
            }).then((res: IPaginatedEntity<Company>) => {
            // @ts-ignore
    ...
    const addAction = (formData: CompanyFormField) => {
        setCrudLoading({...crudLoading, add: true});
        companyRepo.create({input: {...formData}}).then(() => {
    ...
    const editAction = (formData: CompanyFormField) => {
        setCrudLoading({...crudLoading, edit: true});

        const toUpdateData = FormUtils.toUpdateValue<CompanyFormField, Company>(formData, selectedItem);
    ...
    const removeAction = () => {
        ...
        companyRepo.delete({input: {entityId: selectedItem?.id}}).then(() => {
    ...
    const removeMultipleAction = () => {
        ...
        companyRepo.deleteMany({input: {where: {id: {in: ids}}}}).then(() => {
            setCrudLoading({...crudLoading, delete: false});
    ...
    const rows: Array<RowsDef<Company>> = [
        {
            id: 'name',
            align: 'left',
            sort: true,
    ...
    return (<>
            ...
            <GenericModal onClose={() => handleCloseDialog('view')}
                ...
                <div className='w-full p-4'>
                    <CompanyDetails row={selectedItem}/>
                </div>
            </GenericModal>
            ...
            <GenericModal maxWidth={'sm'} minHeight={200}
                ...
                <div className='w-full p-4 mt-16'>
                    <CompanyForm
                        ...
                        initialData={{
                        ...FormUtils.setInitialData<Company, CompanyFormField>(selectedItem)
                    }}
