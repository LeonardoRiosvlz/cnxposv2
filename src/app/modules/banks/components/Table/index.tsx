...
import BanksForm, {BanksFormField} from '../Form';
import BanksDetails from '../Details';
import {Banks} from 'api/repositories/banks/types/banks.types';
...
import {BanksRepository} from "api/repositories/banks/banks.repository";
...
type Props = {}
const BanksTable: React.FC<Props> = (props: Props) => {
    ...
    const banksRepo = useRepository(BanksRepository)
    ...
    const selectedItem = _.first(selected) as Banks;
    ...
    React.useEffect(() => {
        // @ts-ignore
        dispatch(setContext(App_Modules.Banks));
    }, []);
    ...
    const addAction = (formData: BanksFormField) => {
        setCrudLoading({...crudLoading, add: true});
        banksRepo.create({input: {...formData}}).then(() => {
    ...
    const editAction = (formData: BanksFormField) => {
        setCrudLoading({...crudLoading, edit: true});

        const toUpdateData = FormUtils.toUpdateValue<BanksFormField, Banks>(formData, selectedItem);
    ...
    const removeAction = () => {
        ...
        banksRepo.delete({input: {entityId: selectedItem?.id}}).then(() => {
    ...
    const removeMultipleAction = () => {
        ...
        banksRepo.deleteMany({input: {where: {id: {in: ids}}}}).then(() => {
            setCrudLoading({...crudLoading, delete: false});
    ...
    const rows: Array<RowsDef<Banks>> = [
        {
            id: 'name',
            align: 'left',
            sort: true,
            ...
            <GenericModal maxWidth={'sm'} minHeight={200}
                ...
                <div className='w-full p-4 mt-16'>
                    <BanksForm
                        ...
                        initialData={{
                        ...FormUtils.setInitialData<Banks, BanksFormField>(selectedItem)
                    }}
